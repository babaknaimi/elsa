...
SEXP Melsa(SEXP v, SEXP nc, SEXP nr, SEXP nclass, SEXP rr, SEXP cc) {
  int nProtected=0;
  int c, row, col, ngb, q, nnr, nnc, nrow, ncol, cellnr, ncl, n, allNA, nv; 
  ...
  for (c=0;c < n;c++)  {
    ...
    allNA = 0;
    for (j=0;j < nv; j++) {
      if (R_finite(NUMERIC_POINTER(VECTOR_ELT(v,j))[c])) {
        allNA = 1;
        break;
      }
    }
